...
router.post("/add", upload.array("images", 5), async (req, res) => {
  try {
    ...
    if (!owner || !name || !location || !price || !description) {
      console.warn("⚠️ Missing fields in BNB add request.");
    ...
    if (!foundOwner) {
      console.warn("⚠️ Owner not found:", owner);
    ...
    // ✅ Always respond to frontend (prevents “Network Error”)
...
router.get("/owner/:email", async (req, res) => {
  try {
    ...
    const bnbs = await Bnb.find({ owner: owner._id }).populate("owner", "name email");
